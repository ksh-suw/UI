...
function UI:Notify(title, content, time)
    ...
    task.delay(time, function()
        local t = TweenService:Create(box, TweenInfo.new(0.3), {
            BackgroundTransparency = 1
        })
        t:Play()
...
function Window:AddTab(name)
    ...
    function Tab:AddColumn(_, opt)
        ...
        function col:AddSection(title, callback)
            ...
            grid.CellPadding = UDim2.fromOffset(10,6)
            ...
            function sec:AddToggle(text,opt)
                ...
                local row = create("Frame",{Parent=body,BackgroundTransparency=1})
